...
class RiotClient(object):
    def __init__(self, api_key, realm = 'euw', limit=True, max_per_ten_min=500, include_timestamp=True, summonerAPIVersion = "1.4",
        championAPIVersion = "1.2", gameAPIVersion = "1.3", leagueAPIVersion = "2.3", statsAPIVersion = "1.2", teamAPIVersion = "2.2"):
    ...
    def makeapicall(self, remainder, version):
        ...
        url = "https://"+urllib.request.quote(self.realm)+".api.pvp.net/api/lol/" + urllib.request.quote(self.realm) + "/v" + urllib.request.quote(version + remainder) + '?api_key=' + urllib.request.quote(self.api_key)
        print("Calling: %s" % url)
    ...
    def get_summoner_names(self, id_list):
        ...
        output = {}
        for summoner_id, summoner_name in resp.items():
